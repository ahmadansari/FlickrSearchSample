...
#define kReachabilityHostName @"rtsip.neeopal.com" //@"apple.com"

#undef TAG
#define kTAG @"AXNetworkService: "
#define TAG kTAG

#define AXNSLog(TAG, FMT, ...)                                                 \
  if ([[[NSBundle mainBundle] objectForInfoDictionaryKey:@"logsenable"]        \
          boolValue])                                                          \
  NSLog(@"%@" FMT "\n", TAG, ##__VA_ARGS__)
...
static void AXNetworkReachabilityCallback(SCNetworkReachabilityRef target,
  ...
  @autoreleasepool {
    ...
    [[NSNotificationCenter defaultCenter]
        postNotificationOnMainThreadWithName:kAXNetworkEventArgsName
                                      object:eargs];
  }
}
...
- (BOOL)stopHostListening {
  if (mHostReachability) {
    (void)SCNetworkReachabilityUnscheduleFromRunLoop(
        mHostReachability, CFRunLoopGetCurrent(), kCFRunLoopDefaultMode);
    CFRelease(mHostReachability), mHostReachability = NULL;
  }
  return YES;
}
...
- (BOOL)isHostReachable {
  ...
  bool isAvailable = success && (flags & kSCNetworkFlagsReachable) &&
                     !(flags & kSCNetworkFlagsConnectionRequired);
  if (isAvailable) {
    DDLogInfo(@"Host is reachable: %d", flags);
  } else {
    DDLogInfo(@"Host is unreachable");
  }
  return isAvailable;
}
